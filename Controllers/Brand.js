const UserModel = require('../Models/Brand')
...
exports.findbrandCate = (Request, Response) => {
    UserModel.findCatBrand().then(([uMR]) => {
        console.log(uMR)
        responseHelper[200].data = uMR;
        Response.send(responseHelper[200]);
    }).catch(deptError => {
...
exports.findByType = (Request, Response) => {
    ...
    UserModel.findByType(status).then(([uMR]) => {
        responseHelper[200].data = uMR;
        Response.send(responseHelper[200]);
    }).catch(campaignError => {
        sendError(Response, campaignError);
...
exports.groupByType = (Request, Response) => {
    UserModel.groupByType().then(([uMR]) => {
        responseHelper[200].data = uMR;
        Response.send(responseHelper[200]);
    }).catch(campaignError => {
        sendError(Response, campaignError);
